...
// The internal generation endpoints have been removed from the backend to ensure full isolation.
// Generation tasks are strictly delegated via HTTP to the generation engine on port 3010.

// Internal generation routes moved to standalone Generation Engine (Port 3010)
...
const server = app.listen(PORT, () => {
    console.log(`[Conversio API] Server rodando em http://localhost:${PORT}`);
    initAdminDb(); // Run admin tables init on startup
    
    // Iniciar agentes em background APENAS no ENGINE
    if (SERVICE_TYPE === 'ENGINE') {
        setTimeout(() => {
            try {
                MonitorAgent();
                console.log('[SYSTEM] Background services monitoring initiated (ENGINE MODE).');
            } catch (err) {
                console.error('[CRITICAL] Failed to start background agents:', err);
            }
        }, 5000);
    } else {
        console.log('[SYSTEM] Background agents disabled (BACKEND MODE).');
    }
});
