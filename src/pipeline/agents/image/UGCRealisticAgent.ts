...
== BRANDING & TEXT ==
- BRAND COLORS: ${params.useBrandColors ? `Subtly integrate these brand colors into the scene (clothing, lighting, or background): ${JSON.stringify(params.brandColors)}` : 'Use natural and vibrant colors.'}
- TEXT IN IMAGE: ${params.includeText ? 'Include a small, elegant promotional text or price tag in Portuguese (Angola) if it fits the scene.' : 'Do NOT include any text in the image.'}

PRODUCT ANALYSIS: ${params.analysis.substring(0, 2000)}
USER INSTRUCTION: ${params.userPrompt || 'No specific instructions.'}
SEED: ${params.seed}`;
