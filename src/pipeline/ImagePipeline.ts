...
export class ImagePipeline {
    ...
    static async run(options: PipelineOptions) {
        ...
        try {
            ...
            // Get working API Key for KIE.ai (prefer injected key, fallback to DB/ENV)
            ...
            // Log usage
            if (keyId) {
                const modelInDb = this.mapCoreToModel(options.coreId);
                const modelRes = await query('SELECT kie_cost FROM models WHERE style_id = $1 OR name = $2', [modelInDb, modelInDb]);
                const costCredits = Number(modelRes.rows[0]?.kie_cost) || 60; // Default fallback
                await keyManager.logUsage(keyId, 'kie', 'ImagePipeline', 0, 0, costCredits);
            }
    ...
    private static mapCoreToModel(coreId: string): string {
        // Mapping based on user provided list and research
        switch (coreId) {
