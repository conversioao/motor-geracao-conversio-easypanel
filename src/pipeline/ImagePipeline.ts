...
export class ImagePipeline {
    ...
    static async run(options: PipelineOptions) {
        console.log(`[ImagePipeline] 🚀 Starting pipeline for generation: ${options.generationId} (${options.currentIndex}/${options.totalItems})`);
        ...
        try {
            ...
            const { prompt, title, copy, hashtags } = await PromptAgent.generate({
                ...
                contextAntiRepeticao: options.contextAntiRepeticao,
            ...
            // Get working API Key for KIE.ai (use provided or fallback to prioritized DB keys)
            let kieKey = options.apiKey;
            if (!kieKey) {
                const kieKeyObj = await keyManager.getWorkingKey('kie');
                kieKey = kieKeyObj?.key_secret || await getConfig('KIE_AI_API_KEY');
            }

            const taskId = await KieAiNode.createTask({
                model: options.modelId || this.mapCoreToModel(options.coreId),
                prompt,
                imageUrls: options.characterImageUrl ? [options.productImageUrl, options.characterImageUrl] : [options.productImageUrl],
            ...
            const finalImageUrl = await KieAiNode.pollJobStatus(taskId, 10, 5, kieKey, false, false);
            ...
            await query(
                ...
                [finalImageUrl, JSON.stringify({ pipeline_status: 'Concluído', pipeline_progress: 100 }), options.generationId]
            );
            ...
            await query(
                ...
                [JSON.stringify({ error: error.message, pipeline_status: 'Falhou' }), options.generationId]
            );
    ...
    private static mapCoreToModel(coreId: string): string {
        switch (coreId) {
            case 'ugc-lite': return 'google/nano-banana-edit';
            case 'ugc-realistic': return 'nano-banana-pro';
            case 'brand-visual': return 'nano-banana-pro';
            case 'glow-angola': return 'nano-banana-pro';
            case 'impact-ads-pro': return 'seedream/4.5-edit';
            case 'impact-ads-2': return 'nano-banana-2';
