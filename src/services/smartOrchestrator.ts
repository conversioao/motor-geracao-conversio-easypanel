    ...
    for (const plan of plans) {
        try {
            ...
            const existing = await query(`
                ...
                WHERE title = $1 AND status IN ('pending_approval', 'approved', 'executing', 'completed')
            ...
            await query(`
                INSERT INTO orchestrator_action_plans 
                    (type, title, description, priority, target_segment, proposed_actions, estimated_impact, status)
...
async function notifyAdminNewPlans(count: number, plans: any[]) {
    try {
        ...
        const message = `🤖 *ORQUESTRADOR CONVERSIO AI*\n\n` +
            `📋 *${count} Plano(s) de Ação gerado(s) e enfileirado(s) para execução automática*\n` +
            (urgentCount > 0 ? `⚠️ *${urgentCount} plano(s) URGENTE(s)*\n\n` : '\n') +
            `*Resumo dos planos:*\n${summary}\n\n` +
            `👉 _Os planos serão executados autonomamente para manter o sistema optimizado._`;

        await sendPremiumAdminReport(
            adminPhone, 
            'EXECUÇÃO AUTÓNOMA', 
            `🤖 O Orquestrador assumiu ${count} novas tarefas de optimização.`, 
            'Nenhuma acção manual é necessária.', 
...
// ─────────────────────────────────────────────────────────────
// AUTO-HEAL DE AGENTES BLOQUEADOS
// ─────────────────────────────────────────────────────────────

async function autoHealAgents() {
    try {
        const stuckTasks = await query(`
            UPDATE agent_tasks 
            SET status = 'pending', priority = 1 
            WHERE status IN ('running', 'executing') 
            AND created_at < now() - INTERVAL '30 minutes'
            RETURNING id, agent_name, task_type
        `);

        if (stuckTasks.rowCount && stuckTasks.rowCount > 0) {
            console.log(`[SmartOrchestrator] 🛠️ Auto-Heal aplicou reset a ${stuckTasks.rowCount} tarefa(s) bloqueada(s).`);
            
            // Só avisa se forem muitas, para não chatear o admin por 1-2 timeouts
            if (stuckTasks.rowCount > 3) {
                const adminPhone = await getAdminWhatsApp();
                if (adminPhone) {
                    await sendPremiumAdminReport(
                        adminPhone, 
                        'AUTO-HEAL ATIVADO', 
                        `O Orquestrador recuperou ${stuckTasks.rowCount} tarefas presas de vários agentes.`, 
                        'As tarefas foram reiniciadas e colocadas em prioridade alta.', 
                        'info'
                    );
                }
            }
        ...
        console.error('[SmartOrchestrator] Erro ao executar auto-heal:', e);
    }
}
...
export async function executeApprovedPlans() {
    ...
    try {
        ...
        // 2. Buscar planos aprovados e pendentes (agora executa tudo autonomamente)
...
async function executeByPlanType(plan: any, segment: any): Promise<string> {
    ...
    switch (plan.type) {
        ...
        case 'nurture': {
            ...
            return `✅ Campanha "${plan.title}" criada e activada autonomamente (ID: ${campaignId}).\n`;
        }
        ...
        case 'recovery': {
            ...
            for (const lead of leads.rows) {
                await query(`INSERT INTO agent_tasks (agent_name, task_type, status, priority, payload) VALUES ($1, $2, $3, $4, $5)`,
                    ['Agente Recuperação', 'recovery_message', 'pending', 1, JSON.stringify({ userId: lead.user_id, leadId: lead.id, message, source: 'orchestrator_plan', planId: plan.id })]
                ).catch(() => {});
                created++;
            }
        ...
        case 'followup': {
            ...
            for (const lead of leads.rows) {
                await query(`INSERT INTO agent_tasks (agent_name, task_type, status, priority, payload) VALUES ($1, $2, $3, $4, $5)`,
                    ['Agente Envios', 'send_message', 'pending', 2, JSON.stringify({ userId: lead.user_id, message, source: 'orchestrator_plan', planId: plan.id })]
                ).catch(() => {});
                queued++;
            }
...
async function executeAction(action: any, plan: any, segment: any): Promise<string> {
    ...
    switch (actionType) {
        ...
        case 'launch_campaign': {
            ...
            return `✅ Campanha "${action.campaign_name || plan.title}" criada e ACTIVADA AUTONOMAMENTE (ID: ${campaignId}).\n`;
        }
        ...
        case 'start_recovery': {
            ...
            for (const lead of leads.rows) {
                await query(`
                    ...
                    VALUES ($1, $2, $3, $4, $5)
                `, ['Agente Recuperação', 'recovery_message', 'pending', 1, JSON.stringify({
                    userId: lead.user_id,
                    leadId: lead.id,
...
async function notifyAdminExecutionReport(plan: any, report: string, success: boolean) {
    try {
        ...
        // O Orquestrador envia mensagem para TODOS os eventos e planos executados,
        // conforme solicitado pelo admin: "quero ser informado de tudo no sistema pelo whatsapp"
        if (success) {
            await sendPremiumAdminReport(
                adminPhone,
                'EXECUÇÃO AUTÓNOMA CONCLUÍDA',
                `Plano Executado: ${plan.title}`,
                report,
                'info'
            );
        } else {
            await sendPremiumAdminReport(
                adminPhone,
                'FALHA NA EXECUÇÃO AUTÓNOMA',
                `Plano: ${plan.title}`,
                report + '\nAceda ao painel para verificar o erro.',
                'critical'
            );
        }
...
export const runSmartOrchestrator = async () => {
    ...
    try {
        // 0. Tentar recuperar agentes bloqueados antes de iniciar a execução do ciclo
        await autoHealAgents();

        // 1. Executar planos já aprovados pelo Admin (agora automático)
