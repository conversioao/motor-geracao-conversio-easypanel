...
/**
 * Notificações de Eventos de Negócio em Tempo Real
 */
async function checkNewBusinessEvents() {
    const adminWhatsApp = await getAdminWhatsApp();
    if (!adminWhatsApp) return;

    try {
        // 1. Novos Registos (Últimos 5 minutos)
        const newUsers = await query(`
            SELECT id, name, email FROM users 
            WHERE created_at > NOW() - INTERVAL '5 minutes'
            AND (metadata->>'admin_notified')::boolean IS NOT TRUE
        `);

        for (const user of newUsers.rows) {
            await sendWhatsAppMessage(adminWhatsApp, `👤 *NOVO REGISTO*\n\nNome: ${user.name}\nEmail: ${user.email}\nID: ${user.id}\n\nBem-vindo à Conversio AI!`, 'system_alert');
            await query(`UPDATE users SET metadata = COALESCE(metadata, '{}'::jsonb) || '{"admin_notified": true}'::jsonb WHERE id = $1`, [user.id]);
        }

        // 2. Transações / Planos Pagos (Últimos 5 minutos)
        const newSales = await query(`
            SELECT t.id, t.user_id, t.amount, t.status, u.name as user_name 
            FROM transactions t
            JOIN users u ON u.id = t.user_id
            WHERE t.status = 'completed' 
            AND t.created_at > NOW() - INTERVAL '5 minutes'
            AND (t.metadata->>'admin_notified')::boolean IS NOT TRUE
        `);

        for (const sale of newSales.rows) {
            await sendWhatsAppMessage(adminWhatsApp, `💰 *VENDA REALIZADA!*\n\nCliente: ${sale.user_name}\nValor: ${sale.amount} Kz\nID Transação: ${sale.id}\n\nO caixa está a subir! 🚀`, 'system_alert');
            await query(`UPDATE transactions SET metadata = COALESCE(metadata, '{}'::jsonb) || '{"admin_notified": true}'::jsonb WHERE id = $1`, [sale.id]);
        }

        // 3. Planos de Ação Aprovados (Últimos 5 minutos)
        const newPlans = await query(`
            SELECT id, title, type, priority FROM orchestrator_action_plans
            WHERE status = 'approved'
            AND (metadata->>'admin_notified')::boolean IS NOT TRUE
        `);

        for (const plan of newPlans.rows) {
            const priorityLabel = plan.priority === 1 ? '🔴 URGENTE' : '🟢 NORMAL';
            await sendWhatsAppMessage(adminWhatsApp, `🧠 *PLANO APROVADO*\n\nTítulo: ${plan.title}\nTipo: ${plan.type}\nPrioridade: ${priorityLabel}\n\nO Orquestrador está a tratar disto agora.`, 'system_alert');
            await query(`UPDATE orchestrator_action_plans SET metadata = COALESCE(metadata, '{}'::jsonb) || '{"admin_notified": true}'::jsonb WHERE id = $1`, [plan.id]);
        }

    } catch (e: any) {
        console.error('[Agente Monitor] Erro em checkNewBusinessEvents:', e.message);
    }
}
